	...
	public function submitPost($body, $user_to, $imageName) {
		...
		if($check_empty != "") {
			...
			$stopWords = "a about above across after again against all almost alone along already
			 ...
			 area areas around as ask asked asking asks at away b br Br back backed backing backs be became
			 ...
			 things think thinks this those though thats thought thoughts three through
			 ...
			 wants whats was way ways we well wells went were what when where whether
		...
		if(mysqli_num_rows($data_query) > 0) {
			...
			while($row = mysqli_fetch_array($data_query)) {
				...
				$date_time = $row['date_added'];
					...
					if($userLoggedIn == $added_by)
						$delete_button = "<button style='float:right;background:transparent;border:none;' id='post$id'>✕</button>";
					else 
						$delete_button = "";
			...
			if($count > $limit) 
				...
				$str .= "<input type='hidden' class='noMorePosts' value='true'><p style='text-align: center;' class='noMorePostsText'> No more posts to show! </p><img src='assets/images/mood/bottom/bottom.gif'";
		}
